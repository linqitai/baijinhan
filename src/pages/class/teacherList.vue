  ...
  <div class="apply" ref="apply">
    ...
    <div class="operateTableBox">
      ...
      <el-table :data="tableData" border style="width: 100%" v-loading="loading">
        <el-table-column prop="head_img_url" label="头像">
        ...
        <el-table-column label="操作" width="220" fixed="right">
          <template slot-scope="scope">
            <el-button
              @click="selectFreeTimeClick(scope.row)"
              type="text"
              size="small"
              icon="el-icon-edit-outline"
            >选择空闲时间</el-button>
            <el-button
              @click="setClassClick(scope.row)"
              type="text"
              size="small"
              icon="el-icon-edit-outline"
            >排课</el-button>
            ...
            >设置角色</el-button>
            <el-button
              @click="operateBtn(scope.row,'edit')"
              type="text"
              size="small"
              icon="el-icon-edit-outline"
            >编辑</el-button>
    ...
    <el-dialog
      title="设置空余时间"
      :visible.sync="isShowTableSetFreeDialog"
      :append-to-body="true"
      :fullscreen="true"
    ...
    >
      <div class="functionBox">
        <div class="element">
          ...
          <m-time class="margL20" @change="getTimeData" v-model="weekth"></m-time>
        </div>
      </div>
    ...
    <el-dialog
      title="给此教师的空余时间排课"
      :visible.sync="isShowTableSetCourseDialog"
      :append-to-body="true"
      :fullscreen="true"
      :before-close="handleClose"
    >
          ...
          >
            ...
            <div>
              <el-checkbox
                ...
                v-if="coordinatesAttr[checkeds.week][checkeds.hour].check"
                v-model="coordinatesAttr2[checkeds.week][checkeds.hour].check"
              ></el-checkbox>
              <div
                v-if="coordinatesAttr[checkeds.week][checkeds.hour].arranging"
                style="float:left;color:white;margin-left: 6px;"
              >
                <div
                  class="ellipsis width120 margT4"
                >{{coordinatesAttr[checkeds.week][checkeds.hour].course}}</div>
                <div
                  class="ellipsis width120"
                >{{coordinatesAttr[checkeds.week][checkeds.hour].lesson}}</div>
                <div
                  class="ellipsis width120"
                >{{coordinatesAttr[checkeds.week][checkeds.hour].room}}.{{coordinatesAttr[checkeds.week][checkeds.hour].teacher}}</div>
              </div>
          ...
          if (list.length == 0) {
            ...
            for (var i = 0; i < list.length; i++) {
              ...
              if (list[i].arranging == null) {
                ...
                coordinatesAttr[list[i].week][list[i].hour].course =
                  list[i].arranging.course.name;
                coordinatesAttr[list[i].week][list[i].hour].lesson =
                  list[i].arranging.lesson.name;
                coordinatesAttr[list[i].week][list[i].hour].room =
                  list[i].arranging.room.name;
                coordinatesAttr[list[i].week][list[i].hour].teacher =
                  list[i].arranging.teacher.en_name;
              }
            }
